...
class MostrarLibro extends Component {
    ...
    devolverLibro = id => {
        ...
        // Eliminar la persona que esta realizando la devolución
        Swal.fire({
                ...
                try {
                    ...
                    .then( () => {
                        Swal.fire(
                            ...
                            'Se ha realizado la devolución exitosamente.',
                            'success'
                        )
                    })
                } catch(error) {
    ...
    render() { 
        ...
        let cantidadPrestados = (libro.prestados.length > 0) ? <h3 className="my-2">Personas que tienen el libro prestado</h3> : null;

        if(libro.existencia - libro.prestados.length > 0) {
            btnPrestamo = <Link to={`/libros/prestamo/${libro.id}`}
                                className="btn btn-success my-3"
                                >Solicitar Préstamo</Link>
        } else {
            btnPrestamo = null;
        }
